...
add_action( 'admin_post_rudr_duplicate', function() {
	...
	rudr_dowf_duplicate_order( $order_id );
...
add_action( 'woocommerce_order_action_duplicate_order', function( $order ) {
	...
	$new_order_id = rudr_dowf_duplicate_order( $order->get_id() );
...
function rudr_dowf_duplicate_order( $order_id ) {
	...
	$new_order->add_order_note( sprintf( __( 'This order was duplicated from order #%d.', 'duplicate-order-for-woocommerce' ), $order->get_id() ) );
